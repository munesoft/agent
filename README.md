...
To search coding-agent history already indexed by the optional [ctx](https://github.com/ctxrs/ctx)
CLI, use the compatible adapter as the recall source:

```js
const { CodingHistoryAdapter, makeRecallTool } = require("@munesoft/agent");
const ctxHistory = new CodingHistoryAdapter();
if (ctxHistory.available()) agent.addTool(makeRecallTool(ctxHistory));
```
...
The framework now includes optional, zero-dependency building blocks for production-grade agents:

- **Durable workflows** — checkpoint to memory or disk and resume failed workflow nodes with `runDurable()`, `MemoryCheckpointStore`, or `FileCheckpointStore`.
- **Human approval gates** — use `ApprovalPolicy` to allow, deny, or request approval for sensitive tools and tagged actions.
- **Model routing and fallbacks** — `ModelRouter` selects models by policy and automatically falls back when a provider fails or its circuit is cooling down.
- **Observability and evaluations** — `TraceCollector` captures lifecycle events, spans, token usage, cost, and OTLP-shaped exports; `Evaluator` runs weighted quality metrics.
- **Async streaming** — consume agent lifecycle progress and the final response through `agent.streamEvents()` or `streamAgent()` as an async iterator.
- **Schema-first tools** — `defineTool()` accepts JSON Schema or Zod-compatible `safeParse` schemas and validates inputs before handlers run.
- **MCP discovery** — `MCPDiscovery` discovers tools from multiple MCP clients and safely namespaces them in a `ToolRegistry`.
- **Plugins** — `PluginRegistry` installs and removes reusable tools, agents, workflows, memory adapters, and policy bundles.

```js
const {
  ApprovalPolicy, ModelRouter, TraceCollector, Evaluator,
  FileCheckpointStore, runDurable, defineTool,
  MCPDiscovery, PluginRegistry,
} = require("@munesoft/agent");

const approval = new ApprovalPolicy({
  rules: [{ tags: ["destructive"], decision: "ask" }],
  approve: async ({ tool }) => confirmInYourUI(tool.name),
});

const agent = createAgent({ name: "operator", approval, tools: [
  defineTool({
    name: "deploy", description: "Deploy a service",
    jsonSchema: { type: "object", required: ["service"], properties: { service: { type: "string" } } },
    options: { tags: ["destructive"] },
    handler: async ({ service }) => deploy(service),
  }),
] });

for await (const update of agent.streamEvents("deploy billing")) {

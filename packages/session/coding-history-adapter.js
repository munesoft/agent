...
/**
 * @munesoft/agent — coding history adapter
 *
 * Optional bridge to the `ctx` CLI (https://github.com/ctxrs/ctx). Where SessionStore
 * indexes *your framework's own* runs, CodingHistoryAdapter searches the real persisted history
...
class CodingHistoryAdapter {
  ...
  async search(query, opts = {}) {
    if (!this.available()) {
      if (this.debug) console.warn("[CodingHistoryAdapter] `ctx` not found on PATH — returning no results");
      return [];
    }
    ...
    if (r.status !== 0) {
      if (this.debug) console.warn("[CodingHistoryAdapter] ctx search failed:", r.stderr);
      return [];
    }
...
module.exports = { CodingHistoryAdapter };
